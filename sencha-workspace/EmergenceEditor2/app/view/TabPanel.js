...
Ext.define('EmergenceEditor2.view.TabPanel', {
...
//            }
    ...
    ,stateful: true
    ,stateId: 'e3-editorTabs' //Added e3 to avoid issue where my changes to in e1 to e3 effect e3
    ,stateEvents: ['tabchange', 'remove', 'add','afterlayout']
    

    ,getState: function() {
        // TODO: offer all tab types a way to be persisted 
    
        var openFiles = []
            ,file;

        this.items.each(function(item) {
            if(item.isXType('aceeditor')) {
                file = item.getCurrentFile();
                openFiles.push({
                    path: file.path
