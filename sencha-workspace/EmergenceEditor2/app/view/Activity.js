...
Ext.define('EmergenceEditor2.view.Activity', {
    ...
    ,title: 'Activity'
    ,componentCls: 'emergence-activity-panel'
    ,layout: 'fit'
    ,dockedItems: [{
        xtype: 'toolbar'
        ,dock: 'bottom'
        ...
        ,tpl: [
                ...
                ,'<article class="feed-item">'
                    ...
                    ,'<tpl if="EventType != \'delete\'">'
                        ,'<span class="file">'
                            ...
                            ,'<tpl if="FirstAncestorID && RevisionID"><a class="compare" href="#!diff/{CollectionPath}/{Handle}?before=@{FirstAncestorID}&after=@{RevisionID}">compare</a></tpl>'
                        ,'</span>'
                    ,'</tpl>'
            ...
            ,{
                getAvatarUrl: function(userData, size) {
                    if(userData.PrimaryPhotoID) {
                        return '/thumbnail/'+userData.PrimaryPhotoID+'/'+size+'x'+size;
                    }
                    
                    return '//www.gravatar.com/avatar/'+Jarvus.util.MD5.cachedHash((userData.Email||'').toLowerCase())+'?s='+size+'&r=g&d=retro';
                }
            }
        ]
