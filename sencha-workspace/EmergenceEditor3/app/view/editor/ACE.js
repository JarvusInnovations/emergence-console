/*jslint browser: true, undef: true, white: false, laxbreak: true *//*global Ext, EmergenceEditor3, ace*/
Ext.define('EmergenceEditor3.view.editor.ACE', {
	extend: 'Ext.Panel'
	,xtype: 'aceeditor'
...
//        }
	...
	,afterRender: function() {
		...
		if(EmergenceEditor3.app.aceReady)
			Ext.defer(this.initEditor, 10, this);
		else
			EmergenceEditor3.app.on('aceReady', this.initEditor, this, {single: true, delay: 10});
			
		EmergenceEditor3.app.on('aceThemeChanged', this.doChangeTheme, this);
	}
	,initEditor: function() {
...
//			this.loadFile();
	...
	,onEditorChange: function() {
		...
		if(this.loadedValue && this.loadedValue != this.getValue() && !this.fileDirty)
		{
			this.fileDirty = true;
...
//       
//        if(this.isRevision)
//			EmergenceEditor3.API.getRevision(this.path, this.revisionID, this.afterLoadFile, this);
//        else 
//		    EmergenceEditor3.API.getFile(this.path, this.afterLoadFile, this);
//	}
...
//                    
//        EmergenceEditor3.app.fireEvent('afterloadfile', this, this.getRevisionID(), response);
//    }
	,saveFile: function() {
        this.tab.setIcon('/img/loaders/spinner.gif');
		//Check server revision has not changed before uploading file
		EmergenceEditor3.API.getFile(this.path, this.checkServerVersionBeforeSave, this);
		//EmergenceEditor3.API.writeFile('/' + this.path, fileData, this.afterSaveFile, this);
	}
	,checkServerVersionBeforeSave: function(response) {
		if(this.getRevisionID() < response.getResponseHeader('X-VFS-ID')) {
			//Show merge screen
			this.tab.setIcon(this.getIcon(this.contentType)); 
			Ext.Msg.alert('The server version is newer than the file you are trying to save. Save aborted.');
		}
		else {
...
//        this.loadedValue = response.request.options.params;
		console.warn('TODO: update this.loadedFile');
        
        this.tab.setIcon(this.getIcon(this.contentType)); 
        
		this.setRevisionID(response.getResponseHeader('X-Revision'));
    } 
	,getValue: function() {
