...
Ext.define('EmergenceEditor3.plugin.diff.View', {
...
//		me.initDiffViewer();	
	...
	,syncTitles: function() {
		...
		// set titles
		leftTitle = job.beforePath.join('/');
		rightTitle = job.afterPath.join('/');
		
		if(job.beforeRevisionId) {
			leftTitle += '@' + job.beforeRevisionId;
		}
		
		if(job.afterRevisionId) {
			rightTitle += '@' + job.afterRevisionId;
		}
...
//		
//		EmergenceEditor3.API.getRevision(this.path, this.sideAid, this.readyCodeARequestHandler, this);
//		EmergenceEditor3.API.getRevision(this.path, this.sideBid, this.readyCodeBRequestHandler, this);
//	}
...
//	}
	,setCode: function(side, codeLines, diff) {	
		...
		var codeBox = this.down(side=='A' ? '#leftSide' : '#rightSide')
			...
			,pre = codeBox.getTargetEl().insertFirst({tag: 'pre'})
